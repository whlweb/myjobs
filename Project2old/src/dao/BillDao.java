...
public class BillDao extends BaseDAO {
	...
	public List<Bill> query(String q_name,String q_payed) {
		...
		try {
			// （3）获得Statement对象，执行SQL语句
			String sql = "select * from bills where isdeleted=0 and name like '%"+q_name+"%' and payed like '%"+q_payed+"%'";
			Object[] params={};
			ResultSet rs = this.executeSQL(sql, params);
			// （4）处理执行结果(ResultSet)，
			while (rs.next()) {
	...
	public boolean update(Bill bill,String id) {
		boolean flag = false;
		try {
			String sql = "update bills set id=?,name=?, money=?, unit=?, amount=?, description=?, supplier_id=?, payed=? where id="+id;
			Object[] params = { bill.getId(), bill.getName(), bill.getMoney(),
					bill.getUnit(), bill.getAmount(), bill.getDescription(),
					bill.getSupplier_id(), bill.getPayed() };
			int i = this.executeUpdate(sql, params);
			// （4）处理执行结果
			if (i > 0) {
