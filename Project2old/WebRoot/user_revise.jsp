...
<body>
	<%
		...
		if (reviseid != null) {
			...
			if (flag) {
		out.println("<script language=\"javascript\">alert(\"修改成功！\");	window.location=\"userAdmin.jsp\";</script>");
		...
		out.println("<script language=\"javascript\">alert(\"修改失败！\");	history.go(-1);</script>");
			...
			User u = null;
			for (User user : users) {
		if (user.getId() == Integer.parseInt(row != null ? row
				: reviseid)) {
			u = user;
	...
	<div class="main">
		...
		<form method="post" id="myform"
			action="user_revise.jsp?reviseid=${user.id }"
				...
				<table class="box">
					...
					<tr>
						...
						<td><input type="radio" name="role" id="user" value="user"
							/>普通用户 <input type="radio" name="role"
							id="admin" value="admin" />系统管理员</td>
					</tr>
				</table>
			</div>
			<div class="buttons">
