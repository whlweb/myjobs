...
<body>
	<div class="main">
		...
		<form method="post" id="myform" onsubmit="return check()">
			...
			<div class="buttons">
				<a href="user.do?flag=list"  class="input-button">返回</a> 
				<a href="#" class="input-button" onclick="remind()">删除</a>
				<a href="user.do?flag=revise&row=${user.id }" class="input-button">修改</a>
				<a href="user.do?flag=changepw&row=${user.id }" class="input-button">修改密码</a>
			</div>
		</form>
	</div>
	<script type="text/javascript">
		function remind() {
			...
			if (c) {
				location.href = "user.do?flag=delete&row=${user.id }";
			}
		}
		function select() {
			if ('${user.gender}'==1) {
